...
function isAcceptableTitleMatch(candidate, expected) {
  var normalizedCandidate = normalizeTitleForMatch(candidate);
  var normalizedExpected = normalizeTitleForMatch(expected);
  ...
  if (expectedWords.length === 1 && expectedWords[0].length <= 4) {
    return false;
  }

  return normalizedCandidate.indexOf(normalizedExpected + ' ') === 0
    || normalizedExpected.indexOf(normalizedCandidate + ' ') === 0;
}
    ...
    return request(url, { headers: hdrs })
      ...
      .then(function (data) {
        ...
        var filtered = results
          ...
          .filter(function (r) {
            return r.score >= 0.7 && (
              isAcceptableTitleMatch(r.title, title)
              || similarity(r.title, title) >= 0.95
            );
          })
  ...
  return searchPlatform(tmdb.title, tmdb.year, platform, cookie)
    .then(function (hit) {
      ...
      return loadContent(hit.id, platform, cookie).then(function (content) {
        if (!isAcceptableTitleMatch(content.title, tmdb.title)) {
          console.log(PLUGIN_TAG + ' Rejecting loaded title mismatch on ' + PLATFORM_LABEL[platform] + ': "' + content.title + '" vs "' + tmdb.title + '"');
          return null;
        }
